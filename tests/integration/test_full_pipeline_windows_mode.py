"""Integration test for the explicit ``--mode test`` pipeline path."""
...
def _bootstrap_status() -> BootstrapStatus:
    """Build a healthy bootstrap result so the test can focus on orchestration."""
...
def test_explicit_test_mode_pipeline_writes_reports(monkeypatch, tmp_path) -> None:
    """Run the same isolated pipeline through the explicit ``--mode test`` flag.

    The second entry path is important because the project allows both
    ``--test`` and ``--mode test``. Keeping both paths covered ensures the
    application remains predictable for students and for the final report.
    """
    fixed_run_id = "20260507-121500"
